...
module.exports = class SplitChunksPlugin {
	...
	static normalizeOptions(options = {}) {
		return {
			...
			// getCacheGroup是用于处理module的函数，获取module的cacheGroup
	...
	static normalizeChunksFilter(chunks) {
		...
		// chunks选项可以配置为函数，用于匹配符合条件的chunks
			...
			*/
			...
			compilation.hooks.optimizeChunksAdvanced.tap(
				...
				chunks => {
					...
					// step1: Give each selected chunk an index (to create strings from chunks)
					// 后面用于获取模块module对应的chunkId Map<number, Chunk>
					...
					// 获取chunks的chunkId,将chunkId从小到大排序后串成chunksKey(string)。
					const getKey = chunks => {
					...
					// 为了体现不同chunk之间依赖module的联系？？？
					// step2: 创建chunkSetsInGraph, 生成依赖相同module的chunk的所有组合
					/** @type {Map<string, Set<Chunk>>} */
					const chunkSetsInGraph = new Map();
					// compilation.modules 是所有参与打包的文件
					for (const module of compilation.modules) {
						// module.chunksIterable 也是 chunks,哪里生成的？将chunksId按照升序排列，生成chunksKey.
						// module.chunksIterable 对应module依赖的chunks
						// chunksKey 表示module依赖的chunksId
						const chunksKey = getKey(module.chunksIterable);
						// chunkSetsInGraph是Map,用于保存 chunksKey 和 new Set(module.chunksIterable), module的维度
						// chunksKey 相同的情况下怎么处理？chunkKey相同时，直接跳过不做处理，这里主要通过module获取依赖相同module的所有chunks组合,module只是工具。这里是否必须遍历所有module,可能可以优化
					...
					// step3: 将step中生成的chunk组合按照chunk的数量分类
					...
					for (const chunksSet of chunkSetsInGraph.values()) {
						...
						if (array === undefined) {
							array = [];
							// 将 chunk set 按照 size 分组
							chunkSetsByCount.set(count, array);
						}
						array.push(chunksSet);
					}

					// Create a list of possible combinations // Map<chunksKey, Set<Chunk>[]>
					const combinationsCache = new Map(); // Map<string, Set<Chunk>[]>

					const getCombinations = key => {
						//key: chunksKey
						// 根据chunksKey获取当前chunksSet
						const chunksSet = chunkSetsInGraph.get(key);
						var array = [chunksSet];
						if (chunksSet.size > 1) {
							// 遍历chunkSetsByCount,生成chunksSet的子集,即所有可能的chunk组合
					...
					/**
					 * step 4: 遍历所有的module,创建chunksInfoMap
					 *
					 * */
					...
					// 最外层循环遍历module
					for (const module of compilation.modules) {
						// 4.1 Get cache group 获取module对应的cacheGroups配置, // option.test 筛选符合规则的module
						...
						// cacheGroups 当前module对应的所有cacheGroup配置
						// 外层循环 cacheGroups
						for (const cacheGroupSource of cacheGroups) {
							...
							for (const chunkCombination of combs) {
								// 4.3 Break if minimum number of chunks is not reached
								if (chunkCombination.size < cacheGroup.minChunks) continue;
								// Select chunks by configuration chunks:'all'/'async'/'initial
								const {
									chunks: selectedChunks,
					...
					// Filter items were size < minSize // 去掉小于指定minSize的bundle
					...
					// Make sure that maxSize is fulfilled // 先不看
